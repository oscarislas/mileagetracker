...
export default function QuickAddTrip() {
  const [step, setStep] = useState<'collapsed' | 'client' | 'details' | 'success'>('collapsed')
  ...
  if (step === 'success') {
    return (
      <div className="bg-gradient-to-r from-ctp-green/10 to-ctp-teal/10 rounded-xl p-6 border border-ctp-green/20 text-center">
        ...
        <h3 className="text-lg font-semibold text-ctp-text mb-1">Trip Added!</h3>
  ...
  return (
    <div className="bg-ctp-surface0 rounded-xl p-6 border border-ctp-surface1 space-y-4">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className={`h-2 w-2 rounded-full ${step === 'client' ? 'bg-ctp-blue' : 'bg-ctp-green'}`}></div>
        <div className={`h-1 w-8 rounded-full ${step === 'details' ? 'bg-ctp-blue' : step === 'client' ? 'bg-ctp-surface2' : 'bg-ctp-green'}`}></div>
        <div className={`h-2 w-2 rounded-full ${step === 'details' ? 'bg-ctp-blue' : 'bg-ctp-surface2'}`}></div>
      </div>
      ...
      {step === 'client' && (
        <div className="space-y-4">
          <div>
            ...
            {/* Client suggestions */}
            {clientSuggestions?.clients && clientSuggestions.clients.length > 0 && formData.client_name.length > 0 && (
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => formData.client_name.trim() && handleClientSubmit(formData.client_name)}
      ...
      {step === 'details' && (
        <div className="space-y-4">
          <div className="bg-ctp-base rounded-lg p-3 flex items-center gap-2">
            ...
            <span className="text-ctp-text font-medium">{formData.client_name}</span>
            <button 
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!formData.miles || formData.miles <= 0 || createTripMutation.isPending}
