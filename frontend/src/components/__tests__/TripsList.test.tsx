...
vi.mock('../LoadingSkeletons', () => ({
  TripListSkeleton: () => <div data-testid="trip-list-skeleton">Loading trips...</div>,
  TripsEmptyState: () => (
  ...
  ConnectionErrorState: ({ onRetry }: { onRetry?: () => void }) => (
    <div data-testid="connection-error-state">
      ...
      <div>Make sure the backend server is running on http://localhost:8080</div>
...
vi.mock('../../hooks/useTrips', () => ({
  useTrips: (page: number, limit: number, filters?: Record<string, unknown>) => mockUseTrips(page, limit, filters),
}))
...
describe('TripsList', () => {
  ...
  it('displays error state with connection status', () => {
    ...
    expect(screen.getByText('Make sure the backend server is running on http://localhost:8080')).toBeInTheDocument()
  })
  ...
  it('displays no results when filters return empty', () => {
    ...
    expect(screen.getByText('Try adjusting your search criteria or clear filters to see all trips')).toBeInTheDocument()
