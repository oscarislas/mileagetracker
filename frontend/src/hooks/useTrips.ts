...
import type { TripsResponse, CreateTripRequest, UpdateTripRequest, Trip, MessageResponse, TripsQueryParams, TripFilters, DateRangeFilter } from '../types'

// Helper function to convert date range filter to actual dates
function getDateRange(dateRange: DateRangeFilter): { date_from?: string; date_to?: string } {
  if (!dateRange) return {}
  ...
  return useQuery({
    ...
    queryFn: async () => {
      try {
        ...
        const response = await apiClient.get<TripsResponse>('/api/v1/trips', { params })
