...
function SummaryStats({ summaryData }: SummaryStatsProps) {
  ...
  const totalMiles = summaryData.reduce((sum, month) => sum + month.total_miles, 0)
  ...
  const recentAvg = recentMonths.length > 0 
    ? recentMonths.reduce((sum, month) => sum + month.total_miles, 0) / recentMonths.length 
    : 0
  const olderAvg = olderMonths.length > 0 
    ? olderMonths.reduce((sum, month) => sum + month.total_miles, 0) / olderMonths.length 
    : 0
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <StatsCard
        ...
        icon={TruckIcon}
        color="blue"
        trend={Math.abs(trend) > 1 ? { value: Math.round(trend * 10) / 10, label: "recent trend" } : undefined}
      />
    ...
    return (
      <div className="bg-ctp-surface0 rounded-lg p-6 shadow-sm">
        ...
        <div className="text-center py-8">
          ...
          <p className="text-ctp-text text-lg font-medium mb-2">No trips recorded yet</p>
    ...
    <div className="bg-ctp-surface0 rounded-lg p-6 shadow-sm">
      ...
      <div className="space-y-3">
        {sortedMonths.map((month) => {
          const percentage = maxMiles > 0 ? (month.total_miles / maxMiles) * 100 : 0
  ...
  if (isLoading) {
    return (
      <div className="pb-20 px-4 pt-4 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-ctp-surface0 rounded-xl p-4 border border-ctp-surface1 animate-pulse">
        ...
        <div className="bg-ctp-surface0 rounded-lg p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-semibold text-ctp-text">Summary Data</h2>
            </div>
            {connectionStatus && (
          ...
          <div className="text-center py-8">
            ...
            {isConnectionError && (
              <div className="bg-ctp-base rounded-lg p-4 max-w-md mx-auto text-left">
                <h4 className="font-medium text-ctp-text mb-2">🛠️ To start the server:</h4>
                <div className="space-y-1 text-sm font-mono text-ctp-blue">
                  <p><code>make quick-start</code></p>
                  <p>or</p>
