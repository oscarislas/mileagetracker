...
// Define proper types for error handling in tests
interface MockError {
  response?: {
    data?: {
      message?: string;
...
vi.mock("../../utils/errorUtils", () => ({
  getApiErrorMessage: (error: unknown) => {
    ...
    if (mockError?.response?.data?.message)
      return mockError.response.data.message;
    if (mockError?.message) return mockError.message;
...
describe("SettingsPage", () => {
  ...
  it("renders the settings page with title", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("Configure your mileage tracking preferences"),
      ).toBeInTheDocument();
    });
  });
