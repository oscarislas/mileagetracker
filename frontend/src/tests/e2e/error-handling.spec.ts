...
test.describe("Error Handling", () => {
  ...
  test("should handle browser offline state", async ({ page, context }) => {
    ...
    await page.waitForSelector('h1:has-text("Mileage Tracker")', {
      timeout: 10000,
    });
  ...
  test("should handle empty API responses", async ({ page }) => {
    ...
    await expect(
      page.getByText("Start tracking your business mileage"),
    ).toBeVisible();
  });

  test("should handle missing data fields in API response", async ({
    page,
  }) => {
