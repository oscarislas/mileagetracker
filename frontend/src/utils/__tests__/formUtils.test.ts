...
describe("formUtils", () => {
  describe("validateClientName", () => {
    it("should return error for empty client name", () => {
      expect(validateClientName({ clientName: "" })).toBe(
    ...
    it("should return error for client name exceeding max length", () => {
      ...
      expect(validateClientName({ clientName: longName })).toBe(
    ...
    it("should return undefined for valid client name", () => {
      expect(
        validateClientName({ clientName: "Valid Client" }),
      ).toBeUndefined();
    });

    it("should respect custom max length", () => {
      expect(
        validateClientName({ clientName: "Long Name", maxLength: 5 }),
      ).toBe("Client name must be 5 characters or less");
    });
  });
  ...
  describe("validateMiles", () => {
    ...
    it("should respect custom minimum value", () => {
      expect(validateMiles({ miles: 5, minValue: 10 })).toBe(
  ...
  describe("setFieldError", () => {
    it("should set field error", () => {
      const errors = { field1: "Error 1", field2: undefined };
